...
def main():
    ...
    for i in range(len(training_rates)):
        ...
        dataset, train_rate = 'binaryIO/', int(training_rates[i] * 100) # change  for COIL100
        ...
        print "icub tr_rate: ", training_rates[ii], "accuracy", np.count_nonzero(pred == y_t) / float(pred.shape[0]) * 100

        fname = 'nn_results_icub/mlp_icub_pix/'
        sio.savemat(fname + 'prediction_pix_1000_' + str(int(training_rates[ii]*100))+'.mat', mdict={'prediction_pix_1000': pred})
        sio.savemat(fname + 'actual_y_pix_1000_' + str(int(training_rates[ii]*100))+'.mat', mdict={'actual_y_pix_1000': y_t})
